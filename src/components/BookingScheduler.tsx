...
import { Calendar, Clock, User, Phone, ArrowLeft, ArrowRight, CheckCircle, Package, Info, ChevronLeft, ChevronRight, UserCheck } from "lucide-react";
...
function CustomCalendar({ date, onSelect }: { date?: Date; onSelect: (date?: Date) => void }) {
  ...
  return (
    <div className="w-[28rem] border border-white/20 p-6 rounded-lg">
      ...
      <div className="grid grid-cols-7 gap-4 mb-3">
        {weekdays.map(day => (
          <div key={day} className="text-sm text-white/70 uppercase text-center font-bold">
            {day}
          </div>
        ))}
      </div>
        ...
        {paddedDays.map((day, index) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              className={`
                w-16 h-14 rounded-md text-base transition-all duration-300 flex items-center justify-center font-bold transform hover:scale-105
...
export default function BookingScheduler() {
  ...
  function SummaryCard() {
    return (
      <aside className="border border-white/15 bg-white/5 p-4 md:sticky md:top-4">
        <div className="flex items-center gap-2 text-white/70 text-sm mb-2"><Info className="w-4 h-4"/>Resumo</div>
  ...
  function confirm() {
    ...
    const msg = `Oi, meu nome é ${name}! Quero agendar o ${pkg.label} no dia ${d} às ${time}.`;
  ...
  return (
    <div className="bg-white/10 backdrop-blur-md border border-white/15 text-white p-6 md:p-8">
      <div className="mb-6">
      ...
      <div className={`gap-6 ${step < 4 ? 'md:grid md:grid-cols-3' : ''}`}>
        <div className={`${step < 4 ? 'md:col-span-2' : ''} space-y-6 transition-all duration-500 ease-in-out overflow-hidden`}>
      ...
      {step === 1 && (
        <div className="animate-in slide-in-from-right-5 duration-500">
          <div className="flex items-center gap-2 mb-4"><Calendar className="w-4 h-4"/><span>Selecione a data</span></div>
          <div className="text-sm text-white/70 mb-4">* Agendamentos devem ser feitos com 24 horas de antecedência</div>
        ...
        <div className="animate-in slide-in-from-right-5 duration-500">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="flex items-center gap-2">
              ...
              <input value={name} onChange={(e)=>setName(e.target.value)} placeholder="Seu nome" className="w-full bg-white/10 border border-white/20 px-3 py-3 text-white placeholder-white/60 transition-all duration-300 focus:border-white/50 focus:bg-white/15"/>
            </div>
            <div className="flex items-center gap-2">
              <Phone className="w-4 h-4"/>
              <input value={whats} onChange={(e)=>setWhats(formatPhoneBR(e.target.value))} placeholder="Seu WhatsApp" className="w-full bg-white/10 border border-white/20 px-3 py-3 text-white placeholder-white/60 transition-all duration-300 focus:border-white/50 focus:bg-white/15"/>
            </div>
          </div>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="border border-white/15 bg-white/5 p-4 animate-in slide-in-from-right-5 duration-500">
          <div className="flex items-center gap-2 text-amber-400 font-semibold mb-3"><CheckCircle className="w-4 h-4"/>Revise e confirme</div>
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            <div className="space-y-2">
              <div className="font-semibold">Detalhes do Agendamento</div>
              <div>Data: {date?format(date, "d 'de' MMMM 'de' yyyy", {locale:ptBR}):"--"}</div>
              <div>Hora: {time||"--"}</div>
              <div>Serviço: {pkg?.label}</div>
            </div>
            <div className="space-y-2">
              <div className="font-semibold">Dados do Cliente</div>
              <div>Nome: {name}</div>
              <div>Telefone: {whats}</div>
              <div className="font-semibold mt-2">Total: {currency(subtotal)}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={`mt-6 flex items-center ${step === 0 ? 'justify-end' : 'justify-between'}`}>
        {step > 0 && (
        ...
        {step === 4 ? (
          ...
          ((step === 2 && !!time) || (step === 3 && !!name.trim() && whats.replace(/\D/g, "").length === 11)) && (
